...
$motivo = isset($data->motivo) ? trim($data->motivo) : "Sin motivo especificado";
...
try {
    ...
    $query_info = "SELECT nombre FROM planos_bim WHERE id = :id";
    ...
    registrarAuditoriaCompleta(
        $db,
        $usuario['id'],
        "Movió a papelera el plano BIM '{$plano['nombre']}'",
        'planos_bim',
        $plano_id,
        $device_info,
        $motivo
    ...
    echo json_encode([
        "success" => true,
        "message" => "Plano movido a papelera correctamente"
    ]);
    // ============================================
...
try {
    ...
    if ($proyecto) {
        ...
        while ($row = $stmt_usuarios->fetch(PDO::FETCH_ASSOC)) {
            $stmt_notif->execute([
                ':user_id' => $row['user_id'],
                ':mensaje' => "El plano BIM '{$plano['nombre']}' fue eliminado (movido a papelera).",
                ':asunto' => 'Plano eliminado',
